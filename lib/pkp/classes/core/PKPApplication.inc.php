<?php
// Web Fixer: Start output buffering to prevent header issues
if (!headers_sent()) {
    ob_start();
}
...
class PKPApplication {
	...
	// FIXED: PHP4-style constructor diganti dengan __construct
	function __construct() {
		// Configure error reporting - FIXED untuk PHP 7.4/8.0
		...
		if (Config::getVar('general', 'installed')) {
			...
			$conn = DBConnection::getInstance(); // FIXED: Removed reference assignment

			if (!$conn->isConnected()) {
				if (Config::getVar('database', 'debug')) {
					$dbconn = $conn->getDBConn(); // FIXED: Removed reference assignment
	...
	// FIXED: Tambahkan backward compatibility untuk PHP4-style constructor
	...
	function &getApplication() {
		$application =& Registry::get('application'); // KEEP reference assignment - needed for singleton
		return $application;
	}
	 ...
	 */
	function &getRequest() {
		$request =& Registry::get('request', true, null); // KEEP reference assignment - needed for singleton
		...
		if (is_null($request)) {
			...
			$request = new Request();
			Registry::set('request', $request); // Ensure it's set in registry
		}
	...
	function &getDispatcher() {
		$dispatcher =& Registry::get('dispatcher', true, null); // KEEP reference assignment - needed for singleton
		...
		if (is_null($dispatcher)) {
			...
			Registry::set('dispatcher', $dispatcher); // Ensure it's set in registry
	...
	function execute() {
		...
		$dispatcher =& $this->getDispatcher(); // KEEP reference assignment - needed for singleton
	...
	function &getEnabledProducts($category = null, $mainContextId = null) {
		if (is_null($this->enabledProducts) || !is_null($mainContextId)) {
			...
			if ($contextDepth > 0) {
				$request =& $this->getRequest(); // KEEP reference assignment - needed for singleton
				$router =& $request->getRouter(); // KEEP reference assignment - needed for singleton

				if (is_null($mainContextId)) {
					// Try to identify the main context (e.g. journal, conference, press),
					// will be null if none found.
					$mainContext =& $router->getContext($request, 1); // KEEP reference assignment - needed for singleton
			...
			$versionDao =& DAORegistry::getDAO('VersionDAO'); /* @var $versionDao VersionDAO */ // KEEP reference assignment
			$this->enabledProducts =& $versionDao->getCurrentProducts($settingContext); // KEEP reference assignment
		}
	...
	function &getCurrentVersion() {
		$currentVersion =& $this->getEnabledProducts('core'); // KEEP reference assignment
	...
	function getQualifiedDAOName($name) {
		$map =& Registry::get('daoMap', true, $this->getDAOMap()); // KEEP reference assignment for DAO map
	...
	function errorHandler($errorno, $errstr, $errfile, $errline) {
		...
		if(error_reporting() & $errorno) {
			if ($errorno == E_ERROR) {
                ...
                // Web Fixer: Store error for later display
                if (!headers_sent()) {
	...
	function buildErrorMessage($errorno, $errstr, $errfile, $errline) {
		...
		if (!isset($dbServerInfo) && Config::getVar('general', 'installed')) {
			$dbconn = DBConnection::getConn(); // FIXED: Removed reference assignment
	 ...
	 * @param $value mixed
	 */
	// FIXED: Jadikan method ini static
	static function defineExposedConstant($name, $value) {
		define($name, $value);
		assert(preg_match('/^[a-zA-Z_]+$/', $name));
		$constants =& PKPApplication::getExposedConstants(); // KEEP reference assignment for static access
...
function define_exposed($name, $value) {
	...
	if (defined('E_DEPRECATED')) $errorReportingLevel &= ~E_DEPRECATED;
	@error_reporting($errorReportingLevel);
	PKPApplication::defineExposedConstant($name, $value); // FIXED: Sekarang bisa dipanggil static
