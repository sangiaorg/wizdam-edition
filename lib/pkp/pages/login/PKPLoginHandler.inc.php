...
class PKPLoginHandler extends Handler {
	 ...
	 */
	function index($args = array(), $request = null) {
		$this->validate();
	...
	function implicitAuthLogin($args, $request) {
		...
		if (!empty($url) && filter_var($url, FILTER_VALIDATE_URL)) { $request->redirectUrl($url); } else { $request->redirect(null, 'user'); }
	}
	 ...
	 */
	function signIn($args = array(), $request = null) {
		$this->validate();
		$this->setupTemplate($request);
		if (Validation::isLoggedIn()) {
			$request->redirect(null, 'user');
	 ...
	 * Log a user out.
	 */
	function signOut($args = array(), $request = null) {
		$this->validate();
		$this->setupTemplate($request);
		if (Validation::isLoggedIn()) {
			Validation::logout();
	 ...
	 */
	function lostPassword($args = array(), $request = null) {
		$this->validate();
		$this->setupTemplate($request);
		$templateMgr =& TemplateManager::getManager();
	 ...
	 */
	function resetPassword($args = array(), $request = null) {
		$this->validate();
		$this->setupTemplate($request);
		$site =& $request->getSite();
