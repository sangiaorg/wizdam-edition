...
(function($) {
	$.fn.splitter = function(args) {
		...
		return this.each(function() {
			...
			var A = (args.A && args.A.jquery) ? args.A : (args.A && args.A.nodeType === 1 ? $(args.A) : null); // left/top frame
			var B = (args.B && args.B.jquery) ? args.B : (args.B && args.B.nodeType === 1 ? $(args.B) : null); // right/bottom frame
			if (!A || !A.length || !B || !B.length) {
				throw new Error("splitter: options A and B must be DOM elements or jQuery objects.");
			}
